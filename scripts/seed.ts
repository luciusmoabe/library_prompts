...
async function main() {
  ...
  `
  await sql`CREATE INDEX IF NOT EXISTS idx_prompt_tags_tag_id ON prompt_tags(tag_id)`
  ...
  `
  await sql`CREATE INDEX IF NOT EXISTS idx_prompt_versions_prompt_id ON prompt_versions(prompt_id)`
  ...
  await sql`ALTER TABLE prompt_attachments ADD COLUMN IF NOT EXISTS url TEXT`
  ...
  const seedTags = ['Marketing', 'Desenvolvimento', 'Produtividade', 'Pesquisa', 'Produto']
  ...
  if (existingPrompts[0].count === 0) {
    ...
    for (const p of seedPrompts) {
      const [row] = await sql`
        INSERT INTO prompts (title, description, content, purpose, when_to_use, owner_id, favorite)
        VALUES (${p.title}, ${p.description}, ${p.content}, ${p.purpose}, ${p.whenToUse}, ${userIds[p.ownerEmail]}, ${p.favorite})
        RETURNING id
      `
      insertedIds.push(row.id)
      for (const tagName of p.tags) {
        await sql`INSERT INTO prompt_tags (prompt_id, tag_id) VALUES (${row.id}, ${tagIds[tagName]})`
      }
    }
  ...
  console.log(`Seed concluído: ${seedUsers.length} usuários, ${seedTags.length} tags, ${promptCount} prompts, ${attachmentCount} anexos.`)
}
